...
struct apple_smc {
	...
	struct irq_domain *irq_domain_system_state;
};
...
static void apple_smc_handle_notification(struct apple_smc *smc, u64 message)
{
	...
	virq = irq_find_mapping(smc->irq_domain_system_state, 21);
...
static int apple_smc_probe(struct platform_device *pdev)
{
	...
	smc->shmem_res =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "shmem");
	if (IS_ERR(smc->shmem_res))
		return PTR_ERR(smc->shmem_res);
	coproc_res =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "coproc");
	...
	smc->rtk = apple_rtkit_init(dev, smc, coproc_res, "smc-asc",
				    &apple_smc_rtkit_ops);
	if (IS_ERR(smc->rtk))
		return PTR_ERR(smc->rtk);

	ret = apple_rtkit_boot_wait(smc->rtk);
	if (ret)
		return ret;
	...
	smc->irq_domain_system_state = irq_domain_add_linear(
		smc->dev->of_node, 64, &apple_smc_irq_domain_ops, smc);
	if (!smc->irq_domain_system_state)
		return -EINVAL;
